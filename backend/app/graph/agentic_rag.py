"""
Agentic RAG pipeline with self-correction (LangGraph).

Adds two checks on top of basic_rag.py:
1. After retrieval: grade whether the chunks are actually relevant.
   If not, rewrite the question and retry retrieval (up to 2 times).
2. After generation: grade whether the answer is actually grounded in
   the retrieved chunks (not hallucinated). If not, regenerate once.
"""
...
def grade_documents(state: GraphState) -> GraphState:
    ...
    print(f"  [grade_documents] verdict: {verdict} -> relevant={is_relevant}")
...
def rewrite_query(state: GraphState) -> GraphState:
    ...
    print(f"  [rewrite_query] {state['question']} -> {new_question}")
...
def grade_generation(state: GraphState) -> GraphState:
    ...
    print(f"  [grade_generation] verdict: {verdict} -> grounded={is_grounded}")
...
def route_after_grading_documents(state: GraphState) -> str:
    ...
    if state["retrieval_loop_count"] >= MAX_RETRIEVAL_LOOPS:
        print("  [route] max retrieval loops reached, generating anyway with best-effort chunks")
        return "generate"
...
def route_after_grading_generation(state: GraphState) -> str:
    ...
    if state["generation_retry_count"] >= MAX_GENERATION_RETRIES:
        print("  [route] max generation retries reached, returning answer with caveat")
        return END
    return "generate"
...
if __name__ == "__main__":
    ...
    print("FilingSentinel (agentic RAG with self-correction)")
    print("Ask a question about Apple, Tesla, Microsoft, or Amazon 10-K.")
